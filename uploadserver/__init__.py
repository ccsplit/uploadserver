    ...
    def make_file(self):
        if self._binary_file:
            return tempfile.NamedTemporaryFile(
                mode="wb+", dir=args.directory, delete=False
            )
        else:
            return tempfile.NamedTemporaryFile(
                "w+",
...
def validate_token(handler):
    ...
    return (
        ...
        "Token validation successful (no token required)",
    )


def receive_message(handler, parts):
    query_parts = urllib.parse.parse_qs(parts.query)
    if parts.path == "/logger.js" and msg in query_parts:
        with open("logged_messages.txt", "w") as output:
            output.write("{}\n".format(query_parts["msg"]))
...
def receive_upload(handler):
    ...
    for field in fields:
        ...
        if filename:
            ...
            result = (
                http.HTTPStatus.NO_CONTENT,
                "Some filename(s) changed due to name conflict"
                if name_conflict
                else "Files accepted",
            )
...
class SimpleHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        elif parts.path == "/logger.js":
            receive_message(self, parts)
        else:
...
def intercept_first_print():
    if args.server_certificate:
        ...
        def new_print(*args, **kwargs):
            old_print(
                args[0].replace("HTTP", "HTTPS").replace("http", "https"), **kwargs
            )
...
def ssl_wrap(socket):
    ...
    if server_root in server_certificate.parents:
        print(
            'Server certificate "{}" is inside web server root "{}", exiting'.format(
    ...
    if args.client_certificate:
        ...
        if not client_certificate.is_file():
            print(
                'Client certificate "{}" not found, exiting'.format(client_certificate)
            )
            sys.exit(4)

        if server_root in client_certificate.parents:
            print(
                'Client certificate "{}" is inside web server root "{}", exiting'.format(
...
def main():
    ...
    parser.add_argument(
        "--token", "-t", type=str, help="Specify alternate token [default: '']"
    )
    parser.add_argument(
        "--theme",
        type=str,
